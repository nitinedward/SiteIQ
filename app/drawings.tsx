        ...
        <ScrollView style={S.scroll} contentContainerStyle={S.list} showsVerticalScrollIndicator={false}>
          {drawings.length === 0 ? (
            ...
            <TouchableOpacity key={d.id} style={S.row}
              onPress={() => router.push({ pathname: '/drawing/[id]', params: { id: d.id, title: d.title, number: d.number, revision: d.revision, file_url: d.file_url, preview_url: d.preview_url ?? '', project_id: String(project_id), view_only: 'true' } })}
              activeOpacity={0.7}>
