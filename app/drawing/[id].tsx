...
function ToolBtn({ icon, label, active, onPress }: { icon: keyof typeof Ionicons.glyphMap; label: string; active: boolean; onPress: () => void }) {
  return (
    <TouchableOpacity style={[S.toolBtn, active && S.toolBtnOn]} onPress={onPress} activeOpacity={0.75}>
      <Ionicons name={icon} size={24} color={active ? T.indigo : T.mid} />
...
const S = StyleSheet.create({
  ...
  toolBtn:       { flex: 1, alignItems: 'center', justifyContent: 'center', paddingVertical: 12, paddingHorizontal: 3, borderRadius: 13, backgroundColor: T.surface, borderWidth: 1, borderColor: T.line, gap: 3 },
  toolBtnOn:     { backgroundColor: T.indigoSoft, borderColor: T.indigo },
  toolLabel:     { fontSize: 14, color: T.mid, fontWeight: '700' },
